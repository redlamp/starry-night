/** Bottom-left HUD stack (2026-07-26): Seed at the bottom, side-view diagram above it,
 * Performance (badge + stats) on top — and slots collapse when elements hide. */
import { chromium } from "playwright";
import { spawn, type ChildProcess } from "node:child_process";
import { existsSync, writeFileSync, mkdirSync } from "node:fs";
const PORT = 9358;
const url = "http://localhost:7827/?probe=1";
const OUT = process.env.TEMP + "\\stack-shots";
...
const proc: ChildProcess = spawn(
  ...
  [
    ...
    "--user-data-dir=" + process.env.TEMP + "\\stackshot-profile",
...
try {
  ...
  const shot = async (name: string) => {
    const r = await call(ws, "Page.captureScreenshot", {
      format: "png",
      clip: { x: 0, y: 380, width: 340, height: 620, scale: 1 },
    });
    writeFileSync(`${OUT}\\${name}.png`, Buffer.from(String(r.data), "base64"));
    console.log(`${OUT}\\${name}.png`);
  };
  const st = (expr: string) => evalJs(ws, `(window).__sceneStore.getState().${expr}`);

  await st(`setFpsHud(true)`);
  await st(`setPerfStats(true)`);
  await st(`setShowSideView(true)`);
  await sleep(800);
  await shot("1-all-on");
  await st(`setShowSideView(false)`);
  await sleep(500);
  await shot("2-no-diagram");
  await st(`setPerfStats(false)`);
  await sleep(500);
  await shot("3-badge-only");
} finally {
  proc.kill();
}
