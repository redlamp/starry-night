...
- [[decision-camera-v3-continuous-modes]] — **Cam v3: modes as in-camera behaviours (2026-07-15):** `snv3` (new default, forked from SNCv2) folds Top-Down (T = in-camera flight with a banked return pose + 65° deviation re-square), Drift (idle auto-drift, toggle + delay slider, anchored at the current pose), cone-view arc-perpendicular framing (`focusRequest.viewAzimuthDeg`, doubled-angle axial mean + pin/cone pivot glyph), and hand-rolled touch (1-finger orbit, 2-finger pan+pinch, double-tap) into ONE continuous camera — no model swaps; live checklist in [[test-plan-2026-07-15-camera-v3]]
...
- [[decision-camera-v2-retired]] — **Camera v2 retired (2026-09-05):** `snv3` (its 2026-07-15 fork) fully superseded it as default; removed `StarryNightV2Model` + its `snv2` store slice/setter/registry entries, kept the model registry pluggable and `Snv2Config`/`DEFAULT_SNV2` alive as v3's tuning base; persisted `snv2` saves migrate to `snv3`
