...
3. **Eager whole-city build, module-cached** with the shared cache-key recipe (`seed::shape::scale::extent::sketch::deviation::densityProfile`). ~500 ms cold at the 6 km tier (23.5k personas), 0 ms warm. Address pass uses a 150 m spatial hash over road segments (brute force was 19 s).
...
- Directory builds on the main thread on first panel open (~0.5 s worst tier); worker offload if it ever shows.
