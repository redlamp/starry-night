...
**Date:** 2026-07-05. Related: [[decision-merge-styles]].
...
- **Other models (Opus, etc.):** `feat/<detail>` off `dev` → `dev` → `main` (the `feat/` prefix, not `feature/`). Do **not** route through `fable`.
- No feature branches should hang off `fable` between Fable sessions.

Noted after an Opus session (2026-07-05) shipped the inspect focus/selection batch through `fable` by habit; the work itself is fine (it landed in `main`), but the routing was wrong. Cleaned up the merged fable-stream feature branches (`feat/city-features`, `feat/scene-polish`, `feat/drei-camera-tuning`) so none remain active off `fable`.
