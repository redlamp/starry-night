...
3. [ ] `bun scripts/cityGolden.ts` GOLDEN PASS; `bun scripts/personaCheck.ts` 15/15; `bun scripts/heliSplineCheck.ts` PASS.
4. [ ] Writing lab (`/writing-lab`) untouched by this batch — loads, edits persist, export works.
5. [ ] Perf: directory open + columns + arcs at 6 km tier — watch the FPS gauge for regressions.
