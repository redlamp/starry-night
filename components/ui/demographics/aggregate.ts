...
export type DemographicsData = {
  ...
  // (approxCount'd, shown with ~); listed is the exact featured count.
...
export function aggregateDemographics(
  ...
  const s = (n: number) => (scope === "full" ? approxCount(n * scale) : Math.round(n));

  const avgHouseholdSize = listedHouseholds > 0 ? listed / listedHouseholds : 1;

  return {
    header: {
      population: approxCount(truePop),
      listed,
      households: approxCount(truePop / avgHouseholdSize),
      jobs: approxCount(listed > 0 ? (working / listed) * truePop : 0),
    },
    scope,
    scale,
