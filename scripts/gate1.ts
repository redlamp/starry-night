 ...
 *   3. District sanity        — 6..48 districts; every building maps to a real one.
...
function checkSeed(seed: string) {
  ...
  // 3. District sanity.
  if (districts.length < 6 || districts.length > 48) {
    failures.push(`district count ${districts.length} out of [6,48]`);
  }
